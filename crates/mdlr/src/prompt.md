...
- **fan_out**: Dependencies a unit has. High = too many responsibilities — but only flagged when the unit also has real internal complexity. A high-fan_out unit whose cyclomatic and cognitive are both low is a *Delegator* (it just forwards to many callees, usually fine) and is omitted from the listing; inspect it directly with `mdlr check <symbol>` if needed
...
- **inlined_size**: Lines a unit would have if every same-file helper only it calls were folded back in. Catches the one move function_size rewards but the code does not: pushing lines out of a large unit into helpers nothing else calls improves every function_size row without deleting a line. Only listed when the unit absorbs at least `inlined.min_exclusive_fanout` direct helpers nothing else calls (default 3; raise it to 4 for only the widest cases, lower it to 2 to widen the net) AND its own function_size is still below `fair` — a chain of single-caller steps or an already-large unit is omitted. High = the split was cosmetic; fix by deleting code, giving the helpers other callers, or moving them to their own module — NOT by splitting further
