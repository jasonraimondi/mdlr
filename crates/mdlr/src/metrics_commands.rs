...
const METRIC_DESCRIPTIONS: &[(&str, &str)] = &[
    ...
    (
        "inlined_size",
        "Lines a unit would have if every same-file helper only it calls were folded back in. Immune to the one move function_size rewards but the code does not: pushing lines out of a large unit into helpers nothing else calls lowers every function_size row without deleting a line. Listed only when BOTH hold: the unit absorbs at least `inlined.min_exclusive_fanout` direct helpers nothing else calls, 3 by default (absorption alone cannot distinguish a unit that was spread out from a chain of steps calling each other — width can), AND its own function_size is still below 'fair' (on an already-large unit this would restate function_size in a weaker bucket). 'mdlr check <symbol>' ignores the gate. A high value means the split was cosmetic: the work still belongs to one unit. Fix it by deleting code, giving the helpers other callers, or moving them to their own module — not by splitting further.",
    ),
    (
        "params",
